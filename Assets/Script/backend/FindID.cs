...
public class FindID : LoginBase
{
	...
	public void OnClickFindID()
	{
		...
		{
			GuideForIncorrectlyEnteredData(imageEmail, "wrong email.(ex. address@xx.xx)");
			return;
		}
	...
	private void FindCustomID()
	{
		...
		Backend.BMember.FindCustomID(inputFieldEmail.text, callback =>
		{
			...
			{
				SetMessage($"{inputFieldEmail.text} success sending email.");
			}
			// 메일 발송 실패
			else
			{
				...
				switch (int.Parse(callback.GetStatusCode()))
				{
					case 404:   // 해당 이메일의 게이머가 없는 경우
						message = "no user on this email.";
						break;
					case 429:   // 24시간 이내에 5회 이상 같은 이메일 정보로 아이디/비밀번호 찾기를 시도한 경우
						message = "you tried more than 5times in 24hours.";
						break;
					default:
